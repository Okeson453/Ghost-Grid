...
struct RiskConfig
{
    ...
    // Optional / Python-overridable parameters
    double max_daily_loss_pct; // Daily loss as percent of account (optional)
    int max_concurrent;        // Max concurrent positions (optional)
    double max_spread_pct;     // Maximum allowed spread percent (optional)
    double margin_buffer;      // Margin buffer fraction (optional)
};
...
static RiskConfig g_risk_config = {
    ...
    0.0,    // max_daily_loss_pct (unset -> use absolute)
    3,      // max_concurrent (default same as max_positions)
    0.01,   // max_spread_pct (1% default)
    0.0     // margin_buffer
};
// Initialize optional fields defaults
// Note: we'll set optional values after struct literal
// to avoid changing the initializer order in older compilers
